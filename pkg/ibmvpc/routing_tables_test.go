...
src 10.10.1.8 , dst 8.8.8.8: nextHop: 10.10.1.5  (RT : from subnet 10.10.1.0/24)
src 10.10.1.8 , dst 10.10.3.8:   immediate   (RT: system implicit)
src 10.10.3.8 , dst 8.8.8.8: nextHop: PGW (RT: system implicit)


srcAsNextHop: 10.10.1.5, dst: 8.8.8.8: nextHop: FIP (system implicit)
src: 10.10.1.5, dst: 8.8.8.8: nextHop: FIP (system implicit)
src: 10.10.0.5, dst: 8.8.8.8: nextHop: FIP (system implicit)
src: PGW, dst: 8.8.8.8: immeidate
src: FIP, dst: 8.8.8.8: immeidate
...
var testRTAnalyzerTests = []*testRTAnalyzer{
	...
	},
	{
		...
		// TODO: path from 10.10.1.5 -> external address : should be available via another network interface of the VSI (10.10.0.5) and then FIP
		// or is it the proxy ?
		// what about SG? which one is enforced?
	},
