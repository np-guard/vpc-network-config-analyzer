...
// AbstractedNodeSet returns the pip load balancer if it was abstracted
...
func (lb *LoadBalancer) GetLoadBalancerRule(src, dst vpcmodel.Node) *vpcmodel.LoadBalancerRule {
	...
	if slices.Contains(lb.Nodes(), src){
		if !slices.Contains(lb.members(), dst){
			return &vpcmodel.LoadBalancerRule{Denny:true}
		}
		return &vpcmodel.LoadBalancerRule{Denny:false}
	}
	return nil
}
