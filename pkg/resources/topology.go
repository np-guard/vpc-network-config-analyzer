...
// connectivity analysis flow: per VSI: connectivity based on SG and based on NCAL of its subnet
func analyzeConnectivity(t *vpcTopology) {
	for vsi, vsiIP := range t.vsiMap {
		...
		ingressSgConn, egressSgConn := AnalyzeSGListPerInstance(vsiIP, sgObjList)
