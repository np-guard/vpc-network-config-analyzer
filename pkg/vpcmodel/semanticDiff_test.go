...
func configSimpleSubnetDiff() (subnetConfigConn1, subnetConfigConn2 *configConnectivity) {
	...
	extendedConnAll := NewConnWithStatefulGivenTCPStatefulAndNonTCP(connection.All(), connection.All())
	connectionTCP := connection.TCPorUDPConnection(netp.ProtocolStringTCP, 10, 100, 443, 443)
	extendedConnTCP := NewConnWithStatefulGivenTCPStatefulAndNonTCP(connectionTCP, connectionTCP)
