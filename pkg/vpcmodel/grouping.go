...
func (details *srcDstDetails) explanationEncode(c *VPCConfig) string {
	...
	if len(details.actualMergedRules.egressRules) > 0 {
		egressStr = "egress:" + details.actualMergedRules.egressRules.string(c, false) + semicolon
	}
	if len(details.actualMergedRules.ingressRules) > 0 {
		egressStr = "ingress:" + details.actualMergedRules.ingressRules.string(c, true) + semicolon
	}
