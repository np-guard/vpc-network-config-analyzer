...
func checkMissingConn(nodeSet NodeSet, outNodes []VPCResourceIntf, conn, subConn *detailedConn,
	aConn GeneralResponsiveConnectivityMap, info *AbstractionInfo,
	t *testing.T) {
	printGeneralResponsiveConnectivityMap(info.missingEgressConnections)
	printGeneralResponsiveConnectivityMap(info.missingIngressConnections)
...
func TestSimpleAbstraction(t *testing.T) {
	...
	conns := []*detailedConn{detailedConnForAllRsp(), emptyDetailedConn()}
	for _, conn := range conns {
		...
		printGeneralResponsiveConnectivityMap(aConn)
		checkFullConn(nodeSet, outNodes, conn, aConn, info, t)
	}
}

func missingAbstractionConnsPair(conn, subConn *detailedConn, t *testing.T) {
	nodeSet, outNodes := createNodes()
...
func TestMissingAbstraction(t *testing.T) {
	allConns := detailedConnForAllRsp()
	halfConn := detailedConnForResponsive(connection.TCPorUDPConnection(netp.ProtocolStringTCP, 10, 100, 443, 443))
	emptyConns := emptyDetailedConn()
	missingAbstractionConnsPair(allConns, emptyConns, t)
	missingAbstractionConnsPair(allConns, halfConn, t)
	missingAbstractionConnsPair(allConns, emptyConns, t)
}

func printGeneralResponsiveConnectivityMap(conns GeneralResponsiveConnectivityMap) {
	for n1, a := range conns {
		for n2, c := range a {
			fmt.Printf("%s->%s: %s\n", n1.Name(), n2.Name(), c.string())
		}
	}
	fmt.Println("------------------------------------------------")
}
