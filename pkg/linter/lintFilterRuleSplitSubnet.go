...
func (finding *splitRuleSubnet) string() string {
	...
	return fmt.Sprintf("In VPC %s, %s %s rule's indexed %d splits %s. Splitting rule details: %s",
		finding.vpc()[0], finding.rule.Table.LayerName, rule.Table.FilterName, rule.RuleIndex, subnetStr,
...
func (finding *splitRuleSubnet) toJSON() any {
	...
	table := vpcmodel.Table{LayerName: rule.Table.LayerName,
		FilterName: rule.Table.FilterName}
	res := splitRuleSubnetJSON{VpcName: finding.vpcName, Rule: vpcmodel.RuleOfFilter{Table: table, RuleIndex: rule.RuleIndex, RuleDesc: rule.RuleDesc},
