...
func getAllowedXgressConnections(rules []*NACLRule, src, subnetCidr *common.IPBlock, disjointPeers []*common.IPBlock, isIngress bool) map[string]*common.ConnectionSet {
...
func getDisjointPeersForIngressOrEgressAnalysis(
	...
	cidrAll := []*common.IPBlock{(common.NewIPBlockFromCidr("0.0.0.0/0"))}
	cidrSubnet := []*common.IPBlock{subnet}
	if isIngress {
		srcPeers = cidrAll
		dstPeers = cidrSubnet
	} else {
	...
	if isIngress {
		disjointSrcPeers = common.DisjointIPBlocks(srcPeers, []*common.IPBlock{(common.NewIPBlockFromCidr("0.0.0.0/0"))})
		...
		disjointDstPeers = common.DisjointIPBlocks(dstPeers, []*common.IPBlock{(common.NewIPBlockFromCidr("0.0.0.0/0"))})
	}
	return
}
...
func (na *NACLAnalyzer) getNACLRules(naclObj *vpc1.NetworkACL) (ingressRules, egressRules []*NACLRule) {
	ingressRules = []*NACLRule{}
	egressRules = []*NACLRule{}
	for index := range naclObj.Rules {
		rule := naclObj.Rules[index]
...
func (na *NACLAnalyzer) AnalyzeNACL(naclObj *vpc1.NetworkACL, subnet, subnetDisjointTarget *common.IPBlock) (
	ingressRes, egressRes string,
	ingressResConnectivity, egressResConnectivity *ConnectivityResult) {
